// ignore_for_file: provide_deprecation_message, use_key_in_widget_constructors
...
class _DrawerScaffoldState<T> extends State<DrawerScaffold>
  ...
  void updateDrawerState() {
    if (widget.controller != null) {
      if (widget.controller!._open != null) {
        ...
        for (var element in menuControllers!) {
          element.close();
        }
      }
    }
    ...
    if (widget.appBar != null) {
      return AppBar(
          ...
          leading: widget.appBar!.leading ??
              Container(
                  color: ColorsRes.continueShoppingGradient1Color,
                  child: IconButton(
                      icon: SvgPicture.asset(
                          "assets/images/fullApps/foodMaster/drawer.svg",
  ...
  createContentDisplay() {
    ...
    Widget _scaffoldWidget = Scaffold(
    ...
    Widget content = !widget.enableGestures
        ...
        : GestureDetector(
            ...
            onHorizontalDragEnd: (details) {
              if (startDx == -1) return;
              for (var menuController in menuControllers!) {
                if (percentage < 0.5) {
